...
public class MailPicker {
    ...
    public List<ChannelMailVO> concurrentSearch() {
        return this.concurrentSearch(false);
    }

    public List<ChannelMailVO> concurrentSearch(boolean printInfo) {
        ...
        try {
            ...
            List<ChannelMailVO> allChannelMails = new ArrayList<>();
            ...
            for (MailCoordinate coordinate : mailCoordinates) {
                ...
                if (CollectionUtils.isNotEmpty(channelAllMails)) {
                    ChannelMailVO channelMail = new ChannelMailVO()
                            .setUserId(userId)
    ...
    public List<ChannelMailVO> search() {
        return search(false);
    }
     ...
     * 查询邮件
     */
    public List<ChannelMailVO> search(boolean printInfo) {
        try (MailClient mailClient = new MailClient(this.host, this.username, this.password, printInfo)) {
            List<ChannelMailVO> channelMails = new ArrayList<>();
            for (MailCoordinate coordinate : mailCoordinates) {
                List<Mail> mails = mailClient.search(this.getStartDateStr(), this.getEndDateStr(),  coordinate.getFrom(), coordinate.getSubject());
                if (CollectionUtils.isNotEmpty(mails)) {
                    ChannelMailVO channelMail = new ChannelMailVO().setUserId(userId).setChannel(coordinate.getChannel()).setMails(mails);
    ...
    public void addMailCoordinate(RecordParserStrategy<?> recordParserStrategy) {
        if (CollectionUtils.isEmpty(this.mailCoordinates)) {
            this.mailCoordinates = new ArrayList<>();
        }
        this.mailCoordinates.add(new MailCoordinate(recordParserStrategy.getChannel(), recordParserStrategy.getFrom(), recordParserStrategy.getSubject()));
    }
